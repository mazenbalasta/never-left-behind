...
def add_response(message_id: int, response_in: ResponsesIn, repo: MessagesRepo = Depends()):
    return repo.create_response(message_id, response_in)

@router.get("/api/messages/{message_id}/responses", response_model=MessageWithResponsesOut)
def get_messages_with_responses(message_id: int, repo: MessagesRepo = Depends()):
